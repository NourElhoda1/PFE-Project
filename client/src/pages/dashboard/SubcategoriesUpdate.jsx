...
function SubcategoriesUpdate() {
  ...
  const [category_name, setcategory_name] = useState(subcategory?.category_name);
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    AuthAxios.put(`http://localhost:8000/v1/subcategories/`+id, { subcategory_name, category_name, active })
      .then((response) => {
        if (!response.data) {
          console.log('Error updating subcategory');
        }
        dispatch(updateSubcategory({ id, subcategory_name, category_name, active }));
  ...
  return (
    <div className='flex bg-gray-300'>
        ...
        <div className='m-3 flex-1 p-10'>
            <h1 className="text-xl text-gray-900 font-semibold">Update Subcategory</h1>
                        ...
                        <form onSubmit={handleUpdate} className='p-5'>
                            <div className=" gap-6 mt-4 sm:grid-cols-2">
                              <div className="flex flex-col">
                                    <label className="text-gray-700 dark:text-gray-200" htmlFor="category_name">Subcategory Name</label>
                                    <input
                                        id="subcategory_name"
                                        type="text"
